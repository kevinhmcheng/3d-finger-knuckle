...
%session2_matching_protocol
    ...
    for subjectID = (part-1)*ceil(subject_NO/thread)+1:part*ceil(subject_NO/thread)
        ...
        for setID = 1:set_NO
            bwx1 = squeeze(bwx(:,:,subjectID,setID)); %session1
            bwy1 = squeeze(bwy(:,:,subjectID,setID)); %session1
            
            for subjectID_2 = 1:subject_NO
                for setID_2 = 1:set_NO
                    disp([num2str(subjectID) ',' num2str(setID) ',' num2str(subjectID_2) ',' num2str(setID_2) '...'])
                    bwx2 = squeeze(bwx(:,:,subjectID_2,setID_2));%session2
                    bwy2 = squeeze(bwy(:,:,subjectID_2,setID_2));%session2
                    
                     [result] = compute_score(bwx1,bwy1,bwx2,bwy2);
